...
import { collection, getDocs, query, where, addDoc, updateDoc } from "./config";
...
export const getDeviceRegistrations = async (deviceId: string): Promise<DeviceRegistration | null> => {
  try {
    const deviceCollection = collection(db, 'device_registrations');
    const q = query(deviceCollection, where("deviceId", "==", deviceId));
...
export const saveDeviceRegistration = async (registration: DeviceRegistration): Promise<void> => {
  try {
    const deviceCollection = collection(db, 'device_registrations');
    const q = query(deviceCollection, where("deviceId", "==", registration.deviceId));
    const querySnapshot = await getDocs(q);
